  ...
  active.forEach(item=>{
    ...
    if(p){
      if(item.size&&p.sizes){
        ...
        if(sz)sz.stock+=item.qty;
        p.stock=p.sizes.reduce((s,r)=>s+r.stock,0);
      }else{
        p.stock+=item.qty;
      }
      if(dbOnline&&p._sid)sbUpdateOne('products',p._sid,p);
    }
  });
  cacheProducts();
...
function renderReturnHistory(){
  ...
  tb.innerHTML=list.map(r=>{
    ...
    return `<tr>
      ...
      <td>${isAdmin()?`<button class="btn-dl" onclick="deleteReturn(${r.rid})">🗑</button>`:''}</td>
    </tr>`;
  }).join('');
}
function deleteReturn(rid){if(!isAdmin()){showToast('Access denied');return;}
...
function renderExpenseTable(){
  ...
  tb.innerHTML=filtered.map(e=>{
    ...
    return `<tr>
      ...
      <td>${isAdmin()?`<button class="btn-dl" onclick="deleteExpense(${e.eid})">🗑</button>`:''}</td>
    </tr>`;
  }).join('');
}
...
function deleteExpense(eid){if(!isAdmin()){showToast('Access denied');return;}
...
function renderBillsGrid(){
  ...
  grid.innerHTML=filtered.map(s=>{
    ...
    return '<div style="background:var(--card);border:1.5px solid var(--border);border-radius:0;overflow:hidden;box-shadow:var(--shadow-sm);">'
      +'<div style="background:var(--amber);padding:10px 14px;display:flex;justify-content:space-between;align-items:center;">'
      ...
      +'<div style="display:flex;border-top:1px solid var(--border);">'
        +'<button onclick="reprintBill('+s.billNo+')" style="flex:1;padding:11px;background:var(--amber);color:#fff;border:none;font-family:\'DM Mono\',monospace;font-size:10px;cursor:pointer;letter-spacing:1px;">🖨️ REPRINT</button>'
        +(isAdmin()?'<div style="width:1px;background:rgba(255,255,255,0.08);"></div>':'')
        +(isAdmin()?'<button onclick="deleteBill('+s.billNo+')" style="flex:1;padding:11px;background:var(--rust);color:#fff;border:none;font-family:\'DM Mono\',monospace;font-size:10px;cursor:pointer;letter-spacing:1px;">🗑 DELETE</button>':'')
      +'</div>'
    +'</div>';
  }).join('');
}
...
function printRecordReport(){
  ...
  let body='<div style="font-family:Arial,sans-serif;padding:30px;max-width:800px;margin:0 auto;color:#2D3339;">';
  body+='<div style="text-align:center;border-bottom:3px solid #5B8DC9;padding-bottom:16px;margin-bottom:24px;">';
  ...
  body+='<div style="font-size:16px;font-weight:600;margin-top:14px;color:#5B8DC9;">BUSINESS REPORT — '+d.label.toUpperCase()+'</div>';
  ...
  function kpiBox(label,val,color){return '<div style="border:1px solid #ddd;border-radius:0;padding:12px 14px;"><div style="font-size:9px;color:#888;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px;">'+label+'</div><div style="font-size:20px;font-weight:bold;color:'+(color||'#2D3339')+';">'+val+'</div></div>';}

  body+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:24px;">';
  body+=kpiBox('Total Revenue',rs(d.revenue),'#5B8DC9');
  ...
  body+=kpiBox('Avg Bill Value',rs(d.avgBillValue),'#5B8DC9');
  body+='</div>';

  if(d.bestPeriod){
    body+='<div style="background:#EAF2FA;border:1px solid #B8D4EE;border-radius:0;padding:12px 16px;margin-bottom:20px;">';
  ...
  body+=kpiBox('Defective Returns',d.defReturnsCount,'#5B8DC9');
  body+=kpiBox('Loss from Defects',rs(d.totalLoss),'#5B8DC9');
  body+='</div>';
  ...
  const printFooterStyle='<style>@page{margin:15mm 15mm 18mm 15mm;}.print-footer{position:fixed;bottom:0;left:0;right:0;display:flex;justify-content:space-between;font-family:Arial,sans-serif;font-size:9px;color:#999;padding:6px 15mm 0 15mm;border-top:1px solid #ddd;}</style>';
  const printFooter='<div class="print-footer"><span>Designed By AB DEV</span><span>Contact: 03294806508</span></div>';

  const win=window.open('','_blank','width=900,height=900');
  win.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Report — '+d.label+'</title>'+printFooterStyle+'</head><body>'+body+printFooter+'</body></html>');
  win.document.close();
